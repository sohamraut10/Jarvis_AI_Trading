...
function SignalRow({ sig }) {
  ...
  return (
    <div className="flex items-center gap-2 py-1.5 border-b border-gray-800/50 text-xs font-mono">
      <span className="text-gray-600 shrink-0 w-16 truncate">{sig.t ?? ""}</span>
      <span
        className={`shrink-0 px-1.5 py-0.5 rounded border text-[10px] font-bold tracking-widest ${style}`}
      >
        {side || "?"}
      </span>
...
export default function SignalLog({ signals = [] }) {
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded p-4 flex flex-col min-h-0">
      <div className="text-[10px] text-gray-500 tracking-widest uppercase mb-2 shrink-0">
        Signal Log
      </div>
      <div className="overflow-y-auto flex-1 max-h-48 pr-1">
        {signals.length === 0 ? (
          <div className="text-xs text-gray-700 py-4 text-center">
            — awaiting signals —
          </div>
        ) : (
