...
const ASSET_CFG = {
  ...
  "Index Future": "text-emerald-400",
  "Index Option": "text-yellow-400",
  "Stock Option": "text-pink-400",
};
...
function OpportunityRow({ item, onSubscribe }) {
  ...
  const isFuture  = item.asset_class === "Index Future" || item.asset_class === "Stock Future";
    ...
    <div className={`flex items-start gap-2 py-1.5 border-b border-gray-800/40 last:border-0 ${
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 flex-wrap">
          {isOption ? (
            ...
            <>
              <span className="text-xs font-mono font-bold text-emerald-300">{item.underlying || item.symbol}</span>
              <span className="text-[8px] font-bold text-emerald-600 border border-emerald-900/40 px-1 rounded tracking-widest">FUT</span>
              {item.expiry && (
                <span className="text-[8px] text-gray-600">
                  {new Date(item.expiry).toLocaleDateString("en-IN", { day: "2-digit", month: "short" })}
                </span>
        ...
        {(isOption || isFuture) && item.underlying_price != null && (
          <div className="text-[8px] text-gray-700 mt-0.5">
            spot ₹{Number(item.underlying_price).toLocaleString("en-IN")}
            {isOption && item.open_interest != null && ` · OI ${(item.open_interest / 1000).toFixed(0)}K`}
          </div>
        )}
      </div>
      <div className="shrink-0 text-right">
        ...
        <div className="text-[9px] text-gray-600 font-mono">
          {(isOption || isFuture) ? `₹${item.ltp?.toFixed(2)}` : `s${item.score?.toFixed(0)}`}
        </div>
      </div>
